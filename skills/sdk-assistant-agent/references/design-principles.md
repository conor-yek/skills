...
| 文件类型 | 存放内容 | 举例 |
|----------|----------|------|
| `SKILL.md`（入口/Router） | 执行指令、决策树、工具调用步骤 | "Step 1: Glob... Step 2: WebFetch..." |
| `references/drivers/*.md`（驱动 Worker） | 资源索引 + 加载指令，无任何静态知识（无 API 说明、无代码文档、无移植注意点） | 头文件路径、在线文档 URL、示例目录 |
| `references/index.md` | 系统全景图，所有文件的角色与加载时机 | ROUTER/WORKER/REFERENCE/META 分类表 |
| `references/knowledge/*.md` | 经过验证的稳定参考知识 | 错误码表、已知构建问题模式 |
| `references/knowledge/online-docs.md` | URL 映射表 | 各驱动/组件的在线文档地址 |
...
**驱动 Worker 专项（references/drivers/*.md）**
